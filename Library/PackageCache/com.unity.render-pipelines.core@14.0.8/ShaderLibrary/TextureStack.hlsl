...
#define GDRP_ACCESS_STACK(stackName, index) \
    UNITY_GDRP_MATERIAL_LOAD_ARRAY(float4,\
        ...
        index)
#else
#define GDRP_ACCESS_STACK(stackName)
#endif
...
\
GraniteTilesetConstantBuffer GetConstantBuffer_##stackName(uint UNITY_GDRP_MATERIAL_PAGE_OFFSET) \
{ \
...
StackInfo PrepareVT_##stackName(VtInputParameters par, uint UNITY_GDRP_MATERIAL_PAGE_OFFSET)\
{\
...
\
    GraniteTilesetConstantBuffer graniteParamBlock = GetConstantBuffer_##stackName(UNITY_GDRP_MATERIAL_PAGE_OFFSET); \
\
    GraniteConstantBuffers grCB;\
...
#define DECLARE_BUILD_PROPERTIES(stackName, layers, layer0Index, layer1Index, layer2Index, layer3Index)\
    VTProperty BuildVTProperties_##stackName(uint materialOffset)\
    {\
