...
namespace UnityEditor.ShaderGraph.Internal
{
    ...
    internal struct HLSLProperty
    {
        ...
        // Only in CBuffer of GBufferSplat pass, we should generate special version.
        public void AppendTo(ShaderStringBuilder ssb, ref int index)
        {
            ssb.AppendLine(string.Format(
                "UNITY_DEFINE_GDRP_PROP({0}, {1}, {2})",
                kValueTypeStrings[(int)type, (int)precision],
                name,
                index
                ));
            index++;
        ...
        // Only in GBufferSplat pass, we should load variables by using UNITY_GDRP_MATERIAL_LOAD
        // with Offset from CBuffer.
        // Maybe we should support another base shader in ShaderGraph afterwards. By the time,
        // we have to determine whether UNITY_GDRP_MATERIAL_LOAD_ARRAY is in need.
        public void AppendTo(ShaderStringBuilder ssb, bool isVirtualGeometry)
        {
            ssb.AppendLine(string.Format(
                "#define {0} UNITY_GDRP_MATERIAL_LOAD({1}, UNITY_GDRP_MATERIAL_PAGE_OFFSET, UNITY_GDRP_PROP_OFFSET({0}))",
                name,
                kValueTypeStrings[(int)type, (int)precision]
                ));
        }
