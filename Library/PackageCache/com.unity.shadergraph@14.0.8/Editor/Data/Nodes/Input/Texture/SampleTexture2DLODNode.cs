...
namespace UnityEditor.ShaderGraph
{
    ...
    class SampleTexture2DLODNode : AbstractMaterialNode, IGeneratesBodyCode, IMayRequireMeshUV, IMayRequireVG
    {
        ...
        public virtual void GenerateNodeCode(ShaderStringBuilder sb, GenerationMode generationMode)
        {
            ...
            sb.AppendLine("#else");         // else defined(SHADER_API_GLES) && (SHADER_TARGET < 30)
            {
                if ((m_StageCapability & ShaderStageCapability.Fragment) != 0)
                {
                    sb.AppendLine("#ifdef UNITY_GPU_DRIVEN_PIPELINE");

                    var lodVariables = string.Format("$precision lod{0} = CalcLod({1}.tex, IN.ddxddy);", GetVariableNameForSlot(OutputSlotRGBAId), id);
                    sb.AppendLine(lodVariables);

                    var lodResult = string.Format("$precision4 {0} = SAMPLE_TEXTURE2D_LOD({1}.tex, {2}.samplerstate, {1}.GetTransformedUV({3}), lod{0});"
                    ...
                    sb.AppendLine("#else");     // else UNITY_GPU_DRIVEN_PIPELINE
                }
                ...
                if ((m_StageCapability & ShaderStageCapability.Fragment) != 0)
                    sb.AppendLine("#endif");    // UNITY_GPU_DRIVEN_PIPELINE
            }
            sb.AppendLine("#endif");        // defined(SHADER_API_GLES) && (SHADER_TARGET < 30)
