...
namespace UnityEditor.ShaderGraph
{
    ...
    class SampleTexture2DNode : AbstractMaterialNode, IGeneratesBodyCode, IMayRequireMeshUV, IMayRequireVG
    {
		...
		const string kDefaultSampleLodMacro = "SAMPLE_TEXTURE2D_LOD";
		const string kSampleMacroNoBiasLod = "PLATFORM_SAMPLE_TEXTURE2D_LOD";
        ...
        public virtual void GenerateNodeCode(ShaderStringBuilder sb, GenerationMode generationMode)
        {
            ...
            if ((m_StageCapability & ShaderStageCapability.Fragment) != 0)
            {
                ...
                var lodResult = string.Format("$precision4 {0} = {1}({2}.tex, {3}.samplerstate, {2}.GetTransformedUV({4}), lod{0});"
                    , GetVariableNameForSlot(OutputSlotRGBAId)
                    , m_EnableGlobalMipBias ? kDefaultSampleLodMacro : kSampleMacroNoBiasLod
                    , id
