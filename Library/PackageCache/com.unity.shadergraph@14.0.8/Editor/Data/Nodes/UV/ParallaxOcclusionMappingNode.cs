...
return objectScale;");
            ...
            {
                s.AppendLine("// Fix compile bug in UI of ShaderGraph");
                s.AppendLine("#ifndef UNITY_GDRP_MATERIAL_PAGE_OFFSET_PARAMETER");
                s.AppendLine("#ifdef UNITY_GPU_DRIVEN_PIPELINE");
                s.AppendLine("#define UNITY_GDRP_MATERIAL_PAGE_OFFSET_PARAMETER uint UNITY_GDRP_MATERIAL_PAGE_OFFSET");
                s.AppendLine("#else");
                s.AppendLine("#define UNITY_GDRP_MATERIAL_PAGE_OFFSET_PARAMETER");
                s.AppendLine("#endif");
                s.AppendLine("#endif");
                s.AppendLine("#ifndef UNITY_GDRP_MATERIAL_PAGE_OFFSET_ARGUMENT");
                s.AppendLine("#ifdef UNITY_GPU_DRIVEN_PIPELINE");
                s.AppendLine("#define UNITY_GDRP_MATERIAL_PAGE_OFFSET_ARGUMENT UNITY_GDRP_MATERIAL_PAGE_OFFSET");
                s.AppendLine("#else");
                s.AppendLine("#define UNITY_GDRP_MATERIAL_PAGE_OFFSET_ARGUMENT");
                s.AppendLine("#endif");
                s.AppendLine("#endif");
                s.AppendLine("// Required struct and function for the ParallaxOcclusionMapping function:");
                s.AppendLine($"$precision ComputePerPixelHeightDisplacement_{GetVariableNameForNode()}($precision2 texOffsetCurrent, $precision lod, PerPixelHeightDisplacementParam param, TEXTURE2D_PARAM(heightTexture, heightSampler) UNITY_GDRP_MATERIAL_PAGE_OFFSET_PARAMETER)");
                using (s.BlockScope())
                {
...
$precision2 {GetVariableNameForSlot(kParallaxUVsOutputSlotId)} = {heightmap}.GetTransformedUV({tmpUVs}) + ParallaxOcclusionMapping{GetFunctionName()}({lod}, {lodThreshold}, {steps}, {tmpViewDirUV}, {tmpPOMParam}, {tmpOutHeight}, TEXTURE2D_ARGS({heightmap}.tex, {sampler}.samplerstate UNITY_GDRP_MATERIAL_PAGE_OFFSET_ARGUMENT));
