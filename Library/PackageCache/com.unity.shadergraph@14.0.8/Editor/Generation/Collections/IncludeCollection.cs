    ...
    internal class IncludeCollection : IEnumerable<IncludeDescriptor>
    {
        ...
        public IncludeCollection Add(IncludeCollection includes)
        {
            if (includes != null)
            {
                ...
                {
                    AddInternal(include.guid, include.path, include.location, include.fieldConditions, include.shouldIncludeWithPragmas, include.builtinInclude);
                }
            }
            return this;
        }
        ...
        public IncludeCollection Add(string includePath, IncludeLocation location, bool shouldIncludeWithPragmas, bool builtinInclude = false)
        {
            var guid = AssetDatabase.AssetPathToGUID(includePath);
            return AddInternal(guid, includePath, location, null, shouldIncludeWithPragmas, builtinInclude);
        }
        ...
        public IncludeCollection Add(string includePath, IncludeLocation location, FieldCondition[] fieldConditions, bool shouldIncludeWithPragmas, bool builtinInclude)
        {
            var guid = AssetDatabase.AssetPathToGUID(includePath);
            return AddInternal(guid, includePath, location, fieldConditions, shouldIncludeWithPragmas, builtinInclude);
        }

        public IncludeCollection AddInternal(string includeGUID, string includePath, IncludeLocation location, FieldCondition[] fieldConditions = null, bool shouldIncludeWithPragmas = false, bool builtinInclude = false)
        {
            if (string.IsNullOrEmpty(includeGUID))
            {
                ...
                if (existing < 0)
                    includes.Add(new IncludeDescriptor(null, includePath, location, fieldConditions, shouldIncludeWithPragmas, builtinInclude));
                return this;
            }
            else
            {
