...
namespace UnityEditor.ShaderGraph
{
    ...
    internal class TargetPropertyGUIContext : VisualElement
    {
        ...
        public void AddProperty<T>(string label, BaseField<T> field, bool condition, EventCallback<ChangeEvent<T>> evt, bool visible=true)
        {
            if (condition == true)
            {
                AddProperty<T>(label, field, evt, visible);
            }
        }

        public void AddProperty<T>(string label, int indentLevel, BaseField<T> field, bool condition, EventCallback<ChangeEvent<T>> evt, bool visible = true)
        {
            if (condition == true)
            {
                AddProperty<T>(label, indentLevel, field, evt, visible);
            }
        }

        public void AddProperty<T>(string label, BaseField<T> field, EventCallback<ChangeEvent<T>> evt, bool visible = true)
        {
            AddProperty<T>(label, 0, field, evt, visible);
        }

        public void AddProperty<T>(string label, int indentLevel, BaseField<T> field, EventCallback<ChangeEvent<T>> evt, bool visible = true)
        {
            AddProperty<T>(label, string.Empty, indentLevel, field, evt, visible);
        }

        public void AddProperty<T>(string label, string tooltip, int indentLevel, BaseField<T> field, EventCallback<ChangeEvent<T>> evt, bool visible = true)
        {
            ...
            propertyRow.SetEnabled(visible);
