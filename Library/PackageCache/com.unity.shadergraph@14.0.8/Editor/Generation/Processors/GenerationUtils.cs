...
#pragma warning restore 162
        ...
        static ConditionalField[] GetConditionalFieldsFromPixelRequirements(ShaderGraphRequirements requirements)
        {
            return new ConditionalField[]
            {
                ...
                new ConditionalField(StructFields.SurfaceDescriptionInputs.ddxddy,                                      true),
        ...
        internal static void GenerateSurfaceInputStruct(ShaderStringBuilder sb, ShaderGraphRequirements requirements, string structName)
        {
            ...
            using (sb.BlockSemicolonScope())
            {
                ...
                if (requirements.requiresVG)
                {
                    sb.AppendLine($"#ifdef UNITY_GPU_DRIVEN_PIPELINE");
                    sb.AppendLine("float2 {0};", ShaderGeneratorNames.DDXDDY);
                    sb.AppendLine($"#endif");
                }
            }
        }
        ...
        internal static void GenerateSurfaceInputTransferCode(ShaderStringBuilder sb, ShaderGraphRequirements requirements, string structName, string variableName)
            => GenerateSurfaceInputTransferCode(sb, requirements, structName, variableName, ShaderStageCapability.None);

        internal static void GenerateSurfaceInputTransferCode(ShaderStringBuilder sb, ShaderGraphRequirements requirements, string structName, string variableName, ShaderStageCapability stageCapability)
        {
            ...
            // This function will generate both vert and frag stage codes, so we should change it to
            // gain runtime ShaderStageCapability.
            // ShaderStageCapability also includes None and All, but we think that None means codes
            // do not need this part and All is not a runtime stage.
            if (requirements.requiresVG && stageCapability == ShaderStageCapability.Fragment)
            {
                sb.AppendLine($"#ifdef UNITY_GPU_DRIVEN_PIPELINE");
                sb.AppendLine($"{variableName}.{ShaderGeneratorNames.DDXDDY} = IN.{ShaderGeneratorNames.DDXDDY};");
                sb.AppendLine($"#endif");
            }
            else if(requirements.requiresVG && stageCapability == ShaderStageCapability.Vertex)
            {
                sb.AppendLine($"#ifdef UNITY_GPU_DRIVEN_PIPELINE");
                sb.AppendLine($"{variableName}.{ShaderGeneratorNames.DDXDDY} = float2(0.0f, 0.0f);");
                sb.AppendLine($"#endif");
            }
        }
            ...
            bool virtualTextureFeedback = false)
        {
            ...
            if (mode == GenerationMode.VFX)
            {
                ...
                surfaceDescriptionFunction.AppendLine(String.Format("{0} {1}(SurfaceDescriptionInputs IN , uint materialOffset = 0)", surfaceDescriptionName, functionName), false);
            ...
            string graphOutputStructName = k_VertexDescriptionStructName)
        {
            ...
            if (mode == GenerationMode.VFX)
            {
                ...
                builder.AppendLine("{0} {1}({2} IN, uint materialOffset = 0)", graphOutputStructName, functionName, graphInputStructName);
