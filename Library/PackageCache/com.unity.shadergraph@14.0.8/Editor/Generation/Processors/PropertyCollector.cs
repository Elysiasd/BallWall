...
namespace UnityEditor.ShaderGraph
{
    class PropertyCollector
    {
        ...
        public void GetPropertiesDeclaration(ShaderStringBuilder builder, GenerationMode mode, ConcretePrecision defaultPrecision, bool isVirtualGeometry)
        {
            ...
            foreach (var h in hlslProps)
            {
                ...
                {
                    if (isVirtualGeometry)
                        h.AppendTo(builder, ref index);
                    else
                        h.AppendTo(builder);
                }
            ...
            if (isVirtualGeometry)
            {
                ...
                foreach (var h in hlslProps)
                {
                    if (h.declaration == HLSLDeclaration.UnityPerMaterial ||
                        h.declaration == HLSLDeclaration.HybridPerInstance)
                        h.AppendTo(builder, isVirtualGeometry);
                }
            }
